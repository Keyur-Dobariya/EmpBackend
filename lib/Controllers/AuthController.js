...
const updateRecord = async (req, res) => {
    try {
        ...
        Object.keys(updateData).forEach(key => {
            if (!['emailAddress', 'mobileNumber', 'password', 'profilePhoto'].includes(key)) {
                updateFields[key] = updateData[key];
            }
        });

        if (req.file) {
            updateFields.profilePhoto = req.file.path; // Cloudinary returns the `path` field with the uploaded file URL
          }
        ...
        // const uploadSingleFile2 = uploadSingleFile({keyName: 'profilePhoto', folderName: 'profilePictures'});

        // console.log("req.body", uploadSingleFile2);

        // uploadSingleFile2(req, res, (err) => {
        //     if (err) {
        //         return handleError(res, 'Error uploading file', 400);
        //     }
        // });
